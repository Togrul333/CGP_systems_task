...
function collectFormdata(form) {
    let inputs = document.getElementById(form).querySelectorAll('[name]');
    let data = {};
...
function change_status(me, callback) {
    ...
    let mprod_group = me.dataset['product_group'];
    $.ajax({
        type: "POST",
        url: url,
        data: {
            'id': mid,
            'product_group': mprod_group
    },
        success: function (response) {
...
function clear_form(form_name, exclude) {
    document.forms[form_name].querySelectorAll('input,select').forEach(elem => {
        if(exclude && elem.name == exclude)
            return;
        elem.value = "";
    })
